...
(** Enumerates a list using a state transformer to generate indices. The
    initial seed [s] contains the initial state and the function [f]
    transforms this state to compute a new state and an index. These indices
    are sequentially zipped with the given list to produce an indexed list,
    and the final state is returned. *)
let rec enumerate_with_state (s : 's) (f : 's -> 's * 'i) (l : 'a list) :
    's * ('i * 'a) list =
  match l with
  | [] -> (s, [])
  | x :: xs ->
