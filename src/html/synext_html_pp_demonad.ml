...
open Beluga_syntax
open Synext
open Synext_html_pp_state_demonad
...
module Make_html_printer (Html_printing_state : HTML_PRINTING_STATE) = struct
  ...
  open Synext.Make_precedences_demonad (Html_printing_state)
  ...
  let rec pp_lf_kind state kind =
    let open
      Parenthesizer_demonad.Make (Html_printing_state) (Lf_precedence.Ord) in
  ...
  let rec pp_clf_typ state typ =
    let open
      Parenthesizer_demonad.Make (Html_printing_state) (Clf_precedence.Ord) in
  ...
  and pp_comp_expression state expression =
    ...
    | Comp.Expression.Case { scrutinee; check_coverage; branches; _ } ->
        ...
        let pp_check_coverage_pragma_opt state =
          if check_coverage then pp_nop state else pp_space state;
