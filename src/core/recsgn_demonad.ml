...
struct
      ...
      :: entries -> (
        ...
        | entry :: entries -> (
            ...
            let entry_res' =
              try
                Result.ok
                  (with_checkpoint (reconstruct_signature_entry state entry))
              with
  ...
  and reconstruct_signature_pragma state pragma =
    ...
    | Synext.Signature.Pragma.Query
        ...
        freeze_all_unfrozen_declarations state;
        reconstruct_query_pragma state location identifier meta_context typ
  ...
  and reconstruct_query_pragma state location identifier_opt cD extT
      expected_solutions maximum_tries =
    ...
    let cD, apxT = index_lf_query state cD extT in
...
module Signature_reconstruction =
  Make (Recsgn_state_demonad.Signature_reconstruction_state)
