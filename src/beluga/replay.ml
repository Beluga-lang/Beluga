...
module TranscriptRunner = struct
  ...
  *)
  let rpc (request : string) (env : env) : (string * env) Maybe.t =
    ...
    | (_, Left err) ->
       ...
       Nothing
    | (s, Right x) -> Just (x, { env with beluga_out = s } )
  ...
  let run_interaction (i : interaction) (e : env) :
    ...
    let open Maybe in
    let open Either in
    let string_of_line : int Maybe.t -> string =
      function
      | Nothing -> "<unknown>"
...
let rec listify (s : 'a IStream.t) : 'a list =
  let open Maybe in
  match s.IStream.next () with
  | Nothing -> []
  | Just (x, s) -> x :: listify s
