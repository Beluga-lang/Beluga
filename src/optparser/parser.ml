...
let parse spec args =
  let rec go rest_args =
    let go_for_single_option name sub_args cont =
      ...
      | Option.Some (Option.Some arity, fn) ->
          let fn_args, next_args = List.split arity sub_args in
          fn (pp_print_help spec) fn_args;
          cont next_args
      | Option.Some (Option.None, fn) ->
          fn (pp_print_help spec) sub_args;
          cont []
      | Option.None ->
          Result.error
            (`Not_an_option { OptSpec.Error.Option.option_name = name })
    in
    function
