...
module type HTML_PRINTING_STATE = sig
  ...
  include Format_state_demonad.S with type state := state
...
module Html_reference = struct
  ...
  let page reference = reference.page
  ...
  let relative_file_path ~from ~to_ =
    assert (Filename.is_relative from);
    assert (Filename.is_relative to_);
    ...
    let rec go from_dirname_segments to_dirname_segments =
      ...
      | _ ->
          let parent_dir_segments =
            List.rev_map
              (fun _ -> Filename.parent_dir_name)
...
module Entry = struct
  ...
  let id entry = entry.reference.id
