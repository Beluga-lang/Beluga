...
;
  ...
  lf_head:
    [
      [      
         u = UPSYMBOL  ->   LF.Name (_loc, Id.mk_name (Id.SomeString u))

      |
        x = SYMBOL ->  LF.Name (_loc, Id.mk_name (Id.SomeString x))
  ...
  lf_typ_rec:
    [
      [
        b = lf_typ_rec_block ->  b
      | 
        a = lf_typ ->  LF.SigmaLast a
      ]
    ]
  ;
  ...
  clf_ctyp_decl:
    [
      ...
      | 
          ...
          "{";  u = UPSYMBOL; ":"; "["; cPsi = clf_dctx; "."; tA = clf_typ LEVEL "atomic";  "]"; "}" -> 
...
cmp_kind:
 ...
 cmp_rec:
   [[
     f = SYMBOL; ":"; tau = cmp_typ; "="; e = cmp_exp_chk -> 
