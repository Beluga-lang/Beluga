...
module Comp = struct
	...
	type error =
		...
		| PattMismatch    of (I.mctx * meta_obj * meta_typ) * (I.mctx * meta_typ)
		(* | PattMismatch    of (I.mctx * I.dctx * I.normal option * I.tclo) * (I.mctx * I.dctx * I.tclo) *)
...
(* TODO Case - LFcheck stuff *)        
    ...
    | (Hole (loc, f), (tau, t)) ->       
      	Syntax.Ann.Comp.Hole (loc, f, (tau, t))
  ...
  and syn cD (cG,cIH) e : (gctx option * typ * I.msub) = match e with
	...
	| _ -> tau' in 
      if Total.exists_total_decl f then
        (Some cIH, tau, C.m_id, Syntax.Ann.Comp.Var (loc, x, (tau, C.m_id)))
      else
          (None, tau, C.m_id, Syntax.Ann.Comp.Var (loc, x, (tau, C.m_id)))

    | DataConst (loc, c) ->        
        (None,(CompConst.get c).CompConst.typ, C.m_id, Syntax.Ann.Comp.DataConst (loc, c, ((CompConst.get c).CompConst.typ, C.m_id)))

    | DataDest (loc, c) ->	
        (None,(CompDest.get c).CompDest.typ, C.m_id, Syntax.Ann.Comp.DataDest (loc, c, ((CompDest.get c).CompDest.typ, C.m_id)))

    | Const (loc,prog) ->
        if !Total.enabled then
          if (Comp.get prog).Comp.total then
            (None, (Comp.get prog).Comp.typ, C.m_id, Syntax.Ann.Comp.Const (loc, prog, ((Comp.get prog).Comp.typ, C.m_id)))
          else
    ...
    | Apply (loc , e1, e2) ->
        ...
        begin match (tau1, t1) with
          | (TypArr (tau2, tau), t) ->              
              (useIH loc cD cG cIH_opt e2, tau, t, Syntax.Ann.Comp.Apply (loc, e1_ann, (check cD (cG,cIH) e2 (tau2, t)), (tau, t)))
          | (tau, t) ->
...
(* TODO LFcheck stuff *)
    ...
    | PairVal (loc, i1, i2) ->
        ...
        let (tau2,t2)    = C.cwhnfCTyp (tau2, t2) in
          (None, TypCross (TypClo (tau1,t1), TypClo (tau2,t2)), C.m_id, Syntax.Ann.Comp.PairVal (loc, i1_ann, i2_ann, (TypCross (TypClo (tau1,t1), TypClo (tau2,t2)), C.m_id)))


    | Ann (e, tau) ->       
        (None, tau, C.m_id, Syntax.Ann.Comp.Ann ((check cD (cG, cIH) e (tau, C.m_id)), tau, (tau, C.m_id)))
         ...
         let (_, tau2, t2, i2_ann) = syn cD (cG,cIH) i2 in
           if Whnf.convCTyp (tau1,t1) (tau2,t2) then
             begin match Whnf.cwhnfCTyp (tau1,t1) with
               | (TypBox _ , _ ) ->  (None, TypBool, C.m_id, Syntax.Ann.Comp.Equal (loc, i1_ann, i2_ann, (TypBool, C.m_id)))
               | (TypBool, _ )   ->  (None, TypBool, C.m_id, Syntax.Ann.Comp.Equal (loc, i1_ann, i2_ann, (TypBool, C.m_id)))
    ...
    | Boolean b -> (None, TypBool, C.m_id, Syntax.Ann.Comp.Boolean (b, (TypBool, C.m_id)))
