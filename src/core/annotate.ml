...
let useIH loc cD cG cIH_opt e2 = match cIH_opt with
  ...
  (* -> (Annotated.Comp.exp_chk * Syntax.Int.Comp.tclo) *)
 ...
 *)
    ...
    | (Let (loc, i, (x, e)), (tau, t)) ->
	let (_ , i', tau', t') = syn cD (cG,cIH) i in
  ...
  (* -> cIH * Annotated.Comp.exp_syn * Syntax.Int.Comp.tclo *)
  and syn cD (cG,cIH) e : (gctx option * typ * I.msub) = match e with
	...
	| _ -> tau' in
      let e' =
	Annotated.Comp.Var(loc, x, (tau, C.m_id))
      in
      if Total.exists_total_decl f then
	 (Some cIH, e', tau, C.m_id)
      else
	  (None, e', tau, C.m_id)

    | DataConst (loc, c) ->
       let e' =
	 Annotated.Comp.DataConst (loc, c, ((CompConst.get c).CompConst.typ, C.m_id)))
       in
	(None, e', (CompConst.get c).CompConst.typ, C.m_id))

    | DataDest (loc, c) ->
       let e' =
	 Annotated.Comp.DataDest (loc, c, ((CompDest.get c).CompDest.typ, C.m_id)))
       in
	(None, e', (CompDest.get c).CompDest.typ, C.m_id))

    | Const (loc,prog) ->
	if !Total.enabled then
	  if (Comp.get prog).Comp.total then
	    let e' =
	      Annotated.Comp.Const (loc, prog, ((Comp.get prog).Comp.typ, C.m_id))
	    in
	    (None, e', (Comp.get prog).Comp.typ, C.m_id)
	  else
    ...
    | Apply (loc , e1, e2) ->
	let (cIH_opt, e1', tau1, t1) = syn cD (cG,cIH) e1 in
	...
	begin match (tau1, t1) with
	  | (TypArr (tau2, tau), t) ->
	     ...
	     in
	      (useIH loc cD cG cIH_opt e2, e', tau, t)
	  | (tau, t) ->
	      raise (Error (loc, MismatchSyn (cD, cG, e1, VariantArrow, (tau,t))))
	end

    | MApp (loc, e, mC) ->
      let (cIH_opt, e', tau1, t1) = syn cD (cG, cIH) e in
