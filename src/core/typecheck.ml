      ...
      | ((I.App (tM, tS), s1), (I.PiTyp (I.TypDecl (_, tA1), tB2), s2)) ->
         (check cD cPsi (tM, s1) (tA1, s2);
           let
             ...
             B2s = Whnf.whnfTyp (tB2, I.Dot (I.Obj (I.Clo(tM,s1)), s2))
           in 
             checkSpine cD cPsi (tS, s1) B2s sP
          )
    ...
    *)
    and inferHead cD cPsi head = match head with
      ...
      | I.Proj (I.BVar(k'),  target) ->
        let I.Sigma_decl(_, recA) = I.ctxSigmaDec cPsi k'
          (* getType traverses the type from left to right;
             target is relative to the remaining suffix of the type
          *)
        in let rec getType Arecs target j = match (Arecs, target) with
    ...
    *)
    and checkSub cD cPsi s cPsi' = match (cPsi, s, cPsi') with
      ...
      | (cPsi', I.Dot (I.Head(I.BVar w), t), 
                           I.SigmaDec (cPsi, (I.Sigma_decl (_, ARec)))) ->
        ...
        and I.Sigma_decl (_, BRec) = I.ctxSigmaDec cPsi' w
        in
    ...
    and checkSpineK cD cPsi sS1 sA = match (sS1, sA) with
      ...
      | ((I.App (tM, tS), s1), (I.KindPi (I.TypDecl (_, tA1), K), s2)) ->
          (   check cD cPsi (tM, s1) (tA1, s2);
              checkSpineK cD cPsi (tS, s1) (K, I.Dot (I.Obj (I.Clo (tM, s1)), s2))   )
      | ((I.App (tM , tS), _), (tA, s)) ->
          raise  Error ("Expression is applied, but not a function")
    ...
    let rec checkTypRec cD cPsi (recA, s) = match recA with
      ...
      | tA::recA ->
        (checkTyp cD cPsi (tA, s);
         checkTypRec cD (I.DDec(cPsi, I.decSub (I.TypDecl(NONE,tA)) s))
                     (recA, I.dot1 s))

    (* checkKind cD cPsi K succeeds iff cD ; cPsi |- K kind *)
    let rec checkKind cD cPsi K = match K with
          I.Type -> ()
      | I.KindPi(I.TypDecl(x, tA), K) ->
         (checkTyp cD cPsi (tA, I.id); 
          checkKind cD (I.DDec(cPsi, I.TypDecl(x, tA))) K)
