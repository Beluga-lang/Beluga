...
module rec Harpoon_parsers : sig
  ...
  let harpoon_command =
    let by =
      token Token.KW_BY &>
        seq3
          (Comp_parser.comp_expression_object <& token Token.KW_AS)
          identifier
    ...
    and unbox =
      keyword "unbox" &>
        seq2
          ((span Comp_parser.comp_expression_object) <& token Token.KW_AS)
          identifier
  ...
  let harpoon_hypothetical =
    let hypotheses =
      seq3 Meta_parser.meta_context (token Token.PIPE) Comp_parser.comp_context
    in
    seq2
  ...
  let harpoon_directive =
    ...
    and solve =
      keyword "solve" &> Comp_parser.comp_expression_object
      |> span
    ...
    and impossible =
      token Token.KW_IMPOSSIBLE &> Comp_parser.comp_expression_object
      |> span
      $> fun (location, scrutinee) ->
           Harpoon.Directive.Impossible { location; scrutinee }
    and suffices =
      let suffices_branch =
        seq2 Comp_parser.comp_sort_object (braces Harpoon_parsers.harpoon_proof)
        |> span
      ...
      tokens [ Token.KW_SUFFICES; Token.KW_BY ] &>
        seq2
          (Comp_parser.comp_expression_object <& token Token.KW_TOSHOW)
  ...
  let interactive_harpoon_command =
    ...
    and invert =
      keyword "invert" &> Comp_parser.comp_expression_object
      |> span
      $> fun (location, scrutinee) ->
