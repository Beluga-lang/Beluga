...
*)
let genCGoals cD' (LF.Decl (n, mtyp, plicity, inductivity)) cD_tail =
  match mtyp with
  | LF.CTyp _ ->
...
let iterMctx (cD : LF.mctx) (cPsi : LF.dctx) (tA : LF.tclo) : Name.t list =
  ...
  let rec aux acc c =
    function
    | LF.Empty -> acc
    | LF.Dec (cD', LF.Decl(n, LF.ClTyp (LF.MTyp tA', cPsi'), Plicity.Explicit, _))
      | LF.Dec (cD', LF.Decl(n, LF.ClTyp (LF.PTyp tA', cPsi'), Plicity.Explicit, _)) ->
       begin
         try
