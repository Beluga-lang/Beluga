...
*)    
  ...
  *)
  ...
  and invNorm' cD0 ((cvar, offset) as phat, sM, ss, rOccur) = match sM with
    ...
    | (Root (loc, MVar (Offset u, t), _tS (* Nil *)), s (* id *)) ->
        let t' = comp t s (* t' = t, since s = Id *) in
        let _ = dprint (fun () -> "invNorm': Looking for " ^ (R.render_cvar cD0 u) ) in  
  ...
  *)
  ...
  let rec prune  cD0 (phat, sM, ss, rOccur) =
    let _qq : sub = ss in
    let _ = dprint (fun () -> "prune\n"
                      ^ "    " ^ P.normalToString Empty cD0 (Context.hatToDCtx phat) sM ^ "\n"
                      ^ "    " ^ "ss") in
        ...
        in
          match head with
            ...
            | PVar (Offset p, t)   (* tS = Nil,   s = id *) ->
                let (_tA, cPsi1) = Cwhnf.mctxPDec cD0 p in 
                let s' = invSub cD0 (phat, (comp t s, cPsi1), ss, rOccur) in
                  returnHead (PVar (Offset p, s'))
                    
            | Proj (PVar (Offset p, t), i)   (* tS = Nil,   s = id *) ->
                let (_tA, cPsi1) = Cwhnf.mctxPDec cD0 p in 
                let s' = invSub cD0 (phat, (comp t s, cPsi1), ss, rOccur) in
                  returnHead (Proj (PVar (Offset p, s'), i))
  ...
  *)
        ...
        let t' = comp t s1 in
          if isPatSub t' then
            try
              ...
              let sM2' = trail (fun () -> prune cD0 (phat, sM2, ss, MVarRef r))
              in
  ...
  and unifyHead cD0 (phat, head1, head2) = 
      ...
      | ((Comp.TypPiBox ((MDecl(u, tA, cPsi), _ ), tau), t), (Comp.TypPiBox ((MDecl(_, tA', cPsi'), _ ), tau'), t')) -> 
          ...
          let phat   = Context.dctxToHat cPsin in 
            (unifyDCtx cD cPsin cPsin' ; 
             ...
             unifyCompTyp (Dec(cD, MDecl(u, tAn, cPsin))) (tau, Cwhnf.mvar_dot1 t) (tau', Cwhnf.mvar_dot1 t')
            )
      ...
      | Some cnstr ->
          match !cnstr with
            ...
            | Eqn (cD, phat, tM1, tM2) ->
                ...
                unify1 cD (phat, (tM1, id), (tM2, id))
            | Eqh (cD, phat, h1, h2)   ->
                let _ = solveConstraint cnstr in 
                  unifyHead cD (phat, h1, h2) 
