...
module Make_printer_demonad
    (Printing_state : module type of
                        Synext_pp_state_demonad.Printing_state_demonad) =
struct
  ...
  let pp_keyword = pp_string

  let pp_variable = pp_identifier
  ...
  open Synext_precedence_demonad.Make_precedences_demonad (Printing_state)
  ...
  let rec pp_lf_kind state kind =
    let open Parenthesizer_demonad.Make (Printing_state) (Lf_precedence.Ord) in
  ...
  and pp_lf_typ state typ =
    ...
    | LF.Typ.Pi
        ...
        let pp_parameter_type state =
          pp_option state
            (fun stateparameter_type ->
              pp_non_breaking_space state;
              pp_colon state;
              pp_space state;
              pp_lf_typ stateparameter_type)
            parameter_type
        in
        let pp_declaration state =
          pp_parameter_identifier state;
  ...
  let rec pp_clf_typ state typ =
    let open Parenthesizer_demonad.Make (Printing_state) (Clf_precedence.Ord) in
  ...
  and pp_schema state schema =
    let open
      Parenthesizer_demonad.Make (Printing_state) (Schema_precedence.Ord) in
  ...
  and pp_comp_typ state typ =
    let open
      Parenthesizer_demonad.Make (Printing_state) (Comp_sort_precedence.Ord) in
            ...
            | Option.Some operator -> (
                ...
                | Fixity.Infix ->
                    ...
                    let[@warning "-8"] (List1.T
                      ...
                      arguments
                    in
                    pp_hovbox ~indent state (fun state ->
  ...
  and pp_comp_pattern state pattern =
    let open
      Parenthesizer_demonad.Make
        (Printing_state)
        (Comp_pattern_precedence.Ord) in
  ...
  and pp_signature_declaration state declaration =
    ...
    | Signature.Declaration.Typ { identifier; kind; _ } ->
        pp_hovbox state ~indent (fun state ->
            pp_lf_type_constant state identifier;
            pp_non_breaking_space state;
            pp_colon state;
            pp_space state;
    ...
    | Signature.Declaration.Const { identifier; typ; _ } ->
        pp_hovbox state ~indent (fun state ->
            pp_lf_term_constant state identifier;
            pp_non_breaking_space state;
            pp_colon state;
            pp_space state;
            pp_lf_typ state typ;
    ...
    | Signature.Declaration.Schema { identifier; schema; _ } ->
        pp_hovbox state ~indent (fun state ->
            ...
            pp_non_breaking_space state;
            pp_schema_constant state identifier;
            pp_non_breaking_space state;
            pp_equal state;
            pp_space state;
    ...
    | Signature.Declaration.CompTypAbbrev { identifier; kind; typ; _ } ->
        pp_hovbox state ~indent (fun state ->
            ...
            pp_non_breaking_space state;
            pp_computation_abbreviation_constant state identifier;
            pp_non_breaking_space state;
            pp_colon state;
            pp_space state;
    ...
    | Signature.Declaration.Val { identifier; typ; expression; _ } ->
        ...
        in
        let pp_declaration state =
          pp_computation_program state identifier;
    ...
    | Signature.Declaration.Module { identifier; entries; _ } ->
        add_module state identifier (fun state ->
            pp_module_keyword state;
            pp_non_breaking_space state;
            pp_module_constant state identifier;
            pp_non_breaking_space state;
            pp_equal state;
            pp_non_breaking_space state;
  ...
  and pre_add_declaration state declaration =
    ...
    | Signature.Declaration.CompConst { identifier; _ } ->
        add_computation_term_constructor state identifier
  ...
  and pp_grouped_declaration state ~prepend_and declaration =
    ...
    | `Inductive_comp_typ (identifier, kind, constants) ->
        let pp_constant state (identifier, typ) =
          pp_hovbox state ~indent (fun state ->
              pp_pipe state;
              pp_non_breaking_space state;
              pp_computation_constructor state identifier;
              pp_non_breaking_space state;
              pp_colon state;
              pp_space state;
              pp_comp_typ state typ)
        ...
        pp_hovbox state ~indent (fun state ->
            ...
            pp_non_breaking_space state;
            pp_computation_inductive_constant state identifier;
            pp_non_breaking_space state;
            pp_colon state;
            pp_space state;
            pp_comp_kind state kind;
    ...
    | `Stratified_comp_typ (identifier, kind, constants) ->
        ...
        pp_hovbox state ~indent (fun state ->
            ...
            pp_non_breaking_space state;
            pp_computation_stratified_constant state identifier;
            pp_non_breaking_space state;
            pp_colon state;
            pp_space state;
            pp_comp_kind state kind;
    ...
    | `Coinductive_comp_typ (identifier, kind, constants) ->
        let pp_constant state (identifier, observation_typ, return_typ) =
          pp_hovbox state ~indent (fun state ->
              pp_pipe state;
              pp_non_breaking_space state;
              pp_computation_destructor state identifier;
              pp_non_breaking_space state;
              pp_colon state;
              pp_space state;
        ...
        pp_hovbox state ~indent (fun state ->
            ...
            pp_non_breaking_space state;
            pp_computation_coinductive_constant state identifier;
            pp_non_breaking_space state;
            pp_colon state;
            pp_space state;
            pp_comp_kind state kind;
...
module Printer_demonad = struct
  include Synext_pp_state_demonad.Printing_state_demonad
  include
    Make_printer_demonad (Synext_pp_state_demonad.Printing_state_demonad)
end
