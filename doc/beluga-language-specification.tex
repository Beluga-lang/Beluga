...
| `fun' [`|'] <comp-pattern-atomic>+ <thick-forward-arrow> <comp-expression>\\
  (`|' <comp-pattern-atomic>+ <thick-forward-arrow> <comp-expression>)*\\
...
| <dot-qualified-identifier> <comp-expression>* \hfill \texttt{(* Observation *)}\\
...
(`and' <lf-type-constant-declaration>)* `;'
...
| `/' `total' [<totality-order>] `/'\\
| `/' `trust' `/'

<totality-order> ::= \hfill\\
| <argument-totality-order>\\
| <lexical-totality-order>\\
| <simultaneous-totality-order>

<argument-totality-order> ::= \hfill\\
| <integer>\\
| <identifier> `(' <identifier> <identifier>+ `)'

<lexical-totality-order> ::= `{' <identifier>+ `}'

<simultaneous-totality-order> ::= `[' <identifier>+ `]'
...
<documentation-comment-declaration> ::= `%{{' <any>* `}}%'
\end{grammar}
