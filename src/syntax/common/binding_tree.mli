...
(** Abstract definition of a binding tree. *)
module type S = sig
  ...
  (** [is_empty tree] is [true] if and only if there are no bindings in
      [tree]. *)
  val is_empty : 'a t -> Bool.t

  (** [add_toplevel identifier value ?subtree tree] is the binding tree
      derived from [tree] with the addition of [(value, subtree)] bound for
  ...
  (** [add identifier entry ?subtree tree] is the binding tree derived from
      [tree] with the addition of [entry] and [subtree] bound to
      [identifier]. The namespaces of [identifier] must be bound in [tree].
  ...
  (** [lookup_toplevel identifier tree] is [(value, subtree)] where [value]
  ...
  (** [lookup_toplevel_filter identifier p tree] is
      ...
      @raise Unbound_identifier
        if [lookup_toplevel identifier tree] raises it, or if [value] does
        not satisfy [p]. *)
  ...
  (** [maximum_lookup identifiers tree] looks up as many bound identifiers in
      ...
      [maximum_lookup identifiers tree] is one of three outcomes, in
      increasing length of matching path:

      + [`Unbound identifiers] if the first identifier in [identifiers] is
        not bound in [tree], so [identifiers] is not a path in [tree].
      + [`Partially_bound (bound, (entry, subtree), unbound_identifiers)] if
        not all identifiers in [identifiers] are bound in [tree], so first
        few identifiers form a path in [tree].
  ...
  (** [is_identifier_bound identifier tree] is [true] if and only if there is
      a binding for [identifier] in [tree]. *)
  val is_identifier_bound : Identifier.t -> 'a t -> Bool.t

  (** [is_qualified_identifier_bound identifier tree] is [true] if and only
      if there is a binding for [identifier] in [tree]. *)
  val is_qualified_identifier_bound :
    Qualified_identifier.t -> 'a t -> Bool.t

  (** [replace identifier f tree] is the tree derived from [tree] where the
      value bound at [identifier] is [f entry subtree], with
      [(entry, subtree)] being the result of [lookup identifier tree]. *)
  val replace :
  ...
  (** [to_seq tree] is the sequence of values mapped by qualified identifier
      in [tree]. *)
  val to_seq : 'a t -> (Qualified_identifier.t * 'a) Seq.t
end

(** Implementation of a persistent binding tree backed by hash array mapped
    tries. *)
module Hamt : sig
  (** @closed *)
  include S
...
(** Implementation of a persistent binding tree backed by maps. *)
module Map : sig
  (** @closed *)
  include S
...
(** Implementation of a mutable binding tree backed by hash tables. *)
module Hashtbl : sig
  exception Unbound_identifier of Identifier.t
  ...
  (** [add identifier entry ?subtree tree] adds [entry] and [subtree] bound
      to [identifier] in [tree]. The namespaces of [identifier] must be bound
      in [tree].
  ...
  (** [remove identifier tree] removes the binding in [tree] for
      [identifier].
