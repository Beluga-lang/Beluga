...
let rec eval_syn i theta_eta =
        ...
        begin match lookupValue x eta with
          | RecValue ((cid,e'), theta', eta') ->
              ...
              let eta'' = add_mrecs n_list theta' eta' in
                ...
                dprint (fun () -> "  e' = " ^
                          P.expChkToString I.LF.Empty I.LF.Empty (Whnf.cnormExp (e', theta')));
  ...
  | MApp (_, i', (phat, NeutObj h)) ->
      begin match eval_syn i' theta_eta with
        | MLamValue ((_loc, _u, e'), theta1, eta1) ->
            eval_chk e' (I.LF.MDot(I.LF.PObj(phat, Whnf.cnormHead (h, theta)), theta1), eta1)
        | _ -> raise (Violation "Expected MLamValue")
      end
  ...
  let (theta,eta) = theta_eta in
    ...
    match e with
      ...
      | MLam(loc, n, e') ->
          ...
          MLamValue ((loc, n ,e'), Whnf.cnormMSub theta, eta)
      | CtxFun (loc, n, e') ->
          CtxValue ((loc,n,e'), theta, eta)
      | Fun(loc, n, e') ->
          dprint (fun () -> "[FunValue] created: theta = " ^ P.msubToString I.LF.Empty (Whnf.cnormMSub theta));
          FunValue ((loc, n, e'), Whnf.cnormMSub theta, eta)
...
let rec eval e  =
  ...
  Debug.indent 2;
  let result = match eval_chk e (I.LF.MShift 0, Empty) with
