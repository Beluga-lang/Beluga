...
and m_invert s =
  let rec lookup n s p = match s with
    ...
    | MDot (ClObj(_phat, MObj (Root(_, MVar (Offset k, Shift 0), Nil, _))), t') ->
        if k = p then
          Some n
        else lookup (n + 1) t' p
...
and normSub s = match s with
  ...
  | MSVar (n, ((mmvar  (* (_n, {contents = Some (ISub s)}, _cD0, ClTyp (STyp (_, _cPsi), _cPhi), _cnstrs, mDep) *), mt), s')) ->
...
and whnf sM = match sM with
  ...
  (* Meta-variable *)
  | (Root (loc, MVar (Offset _k as u, r), tS, plicity), sigma) ->
  ...
  | (LFHole (loc, id, name), _s) -> (LFHole (loc, id, name), _s)
...
and whnfRedex (sM, sS) = match (sM, sS) with
  ...
  | ((Root (_, _, _, _) as root, s1), (Nil, _s2)) ->
      whnf (root, s1)
...
and conv' sM sN = match (sM, sN) with
  ...
  | ((Root (loc ,AnnH (head, _tA), spine1, plicity), s1), sN) ->
       conv' (Root (loc, head, spine1, plicity), s1) sN

  | (sM, (Root(loc , AnnH (head, _tA), spine2, plicity), s2)) ->
...
(*
  ...
  *)
  ...
  let rec cwhnfCTyp thetaT = match thetaT with
    ...
    | (Comp.TypArr (_, _tT1, _tT2), _t)   -> thetaT

    | (Comp.TypCross (_, _tT1, _tT2), _t)   -> thetaT

    | (Comp.TypPiBox (_, _, _) , _)       -> thetaT
