...
let check_variable_uniqueness variable_kind ctx decl name_of_decl print t =
  ...
  let p d = Name.(name_of_decl d = name) in
...
let split (k : [ `split | `invert | `impossible ]) (i : Comp.exp) (tau : Comp.typ) mfs : t =
  ...
  | `ok ->
     ...
     let make_meta_branch (context, theta, new_state, (cG_p, pat)) =
       ...
       | _ -> Error.raise_violation "[make_meta_branch] pattern not a meta object"
     in
     let make_comp_branch (context, theta, new_state, (cG_p, pat)) =
       ...
       | _ ->
          Error.raise_violation "[get_context_branch] pattern not a constant"
     in
     ...
     let decide_split_kind =
       ...
       | _ -> Error.raise_violation "unhandled pattern type for split"
     in
     match
     ...
     | Some None ->
        Error.raise_violation "mixed cases in split (bug in coverage?)"
...
let unbox (m : Comp.exp) (tau : Comp.typ) (name : Name.t) modifier : t =
  let open Comp in
  solve_by_unbox m (fun cT -> Unbox (m, name, cT, modifier)) tau name modifier

let invoke (i : Comp.exp) (tau : Comp.typ) (name : Name.t) : t =
  let open Comp in
